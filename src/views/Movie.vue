...
export default defineComponent({
  setup() {
    ...
    const dplayer = ref<DPlayer>(null);
    ...
    const touchStart = (e: TouchEvent) => {
      ...
      if (isClick.value) {
        console.log("点击事件");
        console.log(dplayer.value.volume(0.1, true, false));
        // console.log(dplayer.value)
      }
    };
    ...
    const touchEnd = (e: TouchEvent) => {
      ...
      console.log("点击事件结束");
      if (isClick.value) {
        if (dplayer.value.video.paused) {
          dplayer.value.video.play();
        } else {
          dplayer.value.video.pause();
        }
      }

      isClick.value = true;
    };
    //Luminosity API 手机亮度调节
    // useTouch(playerRef,{touchStart,touchMove,touchEnd});
